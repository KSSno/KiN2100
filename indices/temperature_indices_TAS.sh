...
# Run from app05: /../../KiN2100/analyses/github/indices/
## or /../../KiN2100/analyses/indicators/temperature_indices/*_projections
# Remember to run from a screen terminal. This may take a while...
# Copied from testfolder "eqm" 09-dec-2022. Last edited by IBNI 12-apr-2023. 

# CALL:
#../../../github/KiN2100/indices/temperature_indices_TAS.sh   1991   1
#./temperature_indices_TAS.sh   2071   1
# 1) Changed the first input from $mod to $startyr.
# 2) Loop over models. (Disadvantage: it takes a long time to discover faults)
# 3) TODO Go gjennom indeksene.
# 4) TODO rydd opp siste del
# 5) Finn ut om det skal kjøres: 10. mars satte jeg på rcp2.6, eqm_proj, 2041-2070
# 6) TODO gjenta for Tmax Tmin.
#../temperature_indices_TAS-outdated.sh 	noresm-r1i1p1-rca	1
#../temperature_indices_TAS-outdated.sh 	noresm-r1i1p1-remo	1
...
startyr=$1  #1991 #2041 # 2071 1960
endyear=$(( $startyr + 29 ))
echo 'Startyear = ' $startyr ',              endyear= ' $endyear
mod=(  'cnrm-r1i1p1-aladin' 'hadgem-r1i1p1-remo' 'hadgem-r1i1p1-rca' 'noresm-r1i1p1-rca' 'noresm-r1i1p1-remo' 'mpi-r1i1p1-cclm' 'mpi-r2i1p1-remo' 'ecearth-r12i1p1-rca' 'ecearth-r12i1p1-cclm' 'ecearth-r3i1p1-hirham' ) #echo ${mod[0]} # prints the first element
# Example of syntax: $longname=$( 'name1' 'name2' )
# Example:           $shortname=$( 'mn1', 'mn2' )

echo "Argument 1 = " $1       # $startyr  (in the outdated version, $1 was mod)
...
# Argument 3 = $rcp           # Now, this is hard-coded
rcp='rcp26'  # 'hist' 'rcp26' 'rcp45' 'rcp85'
# Argument 4 = $bcm           # Now, this is hard-coded
bcm='eqm_projections'         # 3dbc_projections          # eqm_projections
...
for m in $(seq 0 9); do  
   ...
   echo ${mod[m]}
   filedir='/../../../../ForcingData/BiasAdjust/'$bcmdir'/netcdf/'                 # KiN2100/ForcingData
   savedir='/../../../../analyses/indicators/temperature_indices/'$bcm'/'${mod[m]} # KiN2100/analyses
   echo $savedir
   
   if [ ${mod[m]} = 'cnrm-r1i1p1-aladin' ]; then      # KiN-2015: CNRM-CCLM
      modname=cnrm-r1i1p1-aladin
   elif [ ${mod[m]} = 'ecearth-r12i1p1-cclm' ]; then  # KiN-2015: CNRM-RCA
      modname=ecearth-r12i1p1-cclm
   elif [ ${mod[m]} = 'ecearth-r12i1p1-rca' ]; then   # KiN-2015: EC-CCLM
      modname=ecearth-r12i1p1-rca
   elif [ ${mod[m]} = 'ecearth-r3i1p1-hirham' ]; then # KiN-2015: EC-HIRHAM
      modname=ecearth-r3i1p1-hirham
   elif [ ${mod[m]} = 'hadgem-r1i1p1-rca' ]; then     # KiN-2015: EC-RACMO
       modname=hadgem-r1i1p1-rca
       if [ $endyear = 2100 ]; then
          startyr=2069
          endyear=2098
       fi   
   elif [ ${mod[m]} = 'hadgem-r1i1p1-remo' ]; then    # KiN-2015: EC-RCA
       modname=hadgem-r1i1p1-remo
       if [ $endyear = 2100 ]; then
   	   startyr=2069
	   endyear=2098
       fi
   elif [ ${mod[m]} = 'mpi-r1i1p1-cclm' ]; then       # KiN-2015: HADGEM-RCA
      modname=mpi-r1i1p1-cclm
   elif [ ${mod[m]} = 'mpi-r2i1p1-remo' ]; then       # KiN-2015: IPSL-RCA
      modname=mpi-r2i1p1-remo
   elif [ ${mod[m]} = 'noresm-r1i1p1-rca' ]; then     # KiN-2015: MPI-CCLM
      modname=noresm-r1i1p1-rca
   elif [ ${mod[m]} = 'noresm-r1i1p1-remo' ]; then    # KiN-2015: MPI-RCA
   ...
   for y in $years; do
       ...
       if [ $2 = "1" ]; then
   	   ...
   	   if [ $rcp = "hist" ]; then
	       ...
	       # [1] /../../KiN2100/ForcingData/BiasAdjust/3dbc-eqm/netcdf//hadgem-r1i1p1-rca/tas/hist/*2014.n
...
done   # end looping over models  (for m in $(seq 0 9); do)
