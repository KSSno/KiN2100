...
### Filelists
# near future
filelist_wind_rcp26_nf <- list.files("/lustre/storeB/project/KSS/kin2100_2024/Indices/30yr-diff-files_for_plotting/sfcWind/","30yrmean_nf-change.*rcp26",full.names = T)
filelist_wind_rcp45_nf <- list.files("/lustre/storeB/project/KSS/kin2100_2024/Indices/30yr-diff-files_for_plotting/sfcWind/","30yrmean_nf-change.*rcp45",full.names = T)
filelist_wind_ssp370_nf <- list.files("/lustre/storeB/project/KSS/kin2100_2024/Indices/30yr-diff-files_for_plotting/sfcWind/","30yrmean_nf-change.*ssp370",full.names = T)

# far future
filelist_wind_rcp26_ff <- list.files("/lustre/storeB/project/KSS/kin2100_2024/Indices/30yr-diff-files_for_plotting/sfcWind/","30yrmean_ff-change.*rcp26",full.names = T)
filelist_wind_rcp45_ff <- list.files("/lustre/storeB/project/KSS/kin2100_2024/Indices/30yr-diff-files_for_plotting/sfcWind/","30yrmean_ff-change.*rcp45",full.names = T)
filelist_wind_ssp370_ff <- list.files("/lustre/storeB/project/KSS/kin2100_2024/Indices/30yr-diff-files_for_plotting/sfcWind/","30yrmean_ff-change.*ssp370",full.names = T)
...
if(READ)
{
  ...
  for(i in 1:nmod)
  {
    ...
    Zrcp26[1,i] <- mean(ncvar_get(nc,"sfcWind")*mask,na.rm=T)
    nc_close(nc)
    nc <- nc_open(filelist_wind_rcp26_ff[i])
    Zrcp26[2,i] <- mean(ncvar_get(nc,"sfcWind")*mask,na.rm=T)
    nc_close(nc)
    
    #RCP45
    nc <- nc_open(filelist_wind_rcp45_nf[i])
    Zrcp45[1,i] <- mean(ncvar_get(nc,"sfcWind")*mask,na.rm=T)
    nc_close(nc)
    nc <- nc_open(filelist_wind_rcp45_ff[i])
    Zrcp45[2,i] <- mean(ncvar_get(nc,"sfcWind")*mask,na.rm=T)
    nc_close(nc)
    
    #SSP370
    nc <- nc_open(filelist_wind_ssp370_nf[i])
    Zssp370[1,i] <- mean(ncvar_get(nc,"sfcWind")*mask,na.rm=T)
    nc_close(nc)
    nc <- nc_open(filelist_wind_ssp370_ff[i])
    Zssp370[2,i] <- mean(ncvar_get(nc,"sfcWind")*mask,na.rm=T)
    nc_close(nc)
  }
}
...
#SSP370
plot(Zssp370[1,],xlim=c(0,45),ylim=c(-5,5),pch="+")
points(22:41,Zssp370[2,],pch="+")
abline(h=0)

#RCP4.5
plot(Zrcp45[1,],xlim=c(0,45),ylim=c(-5,5),pch="+")
points(22:41,Zrcp45[2,],pch="+")
abline(h=0)

#RCP2.6
plot(Zrcp26[1,],xlim=c(0,45),ylim=c(-5,5),pch="+")
...
png("/home/andreasd/Documents/KiN2100_2023/Indices/plots/sfcWind/sfcWind_changes_ANN.png",width = k*1280, height = k*640,pointsize=k*28)
...
plot(1951:2110,rep(0,160),xlim=c(1965,2090),ylim=yl,xaxt="n",type="l",lwd=k,lty=3,xlab="",ylab="Avvik (%)",col="grey")
abline(h=seq(-10,10,1),col="grey",lty=3,lwd=k)
...
nc <- nc_open("/lustre/storeB/users/andreasd/KiN_2023_data/klinogrid/mergetime_fldmean_yearmean_KliNoGrid_sfcWind.nc")
sfcWind_ref_i <- ncvar_get(nc,"windspeed_10m")
nc_close(nc)

sfcWind_ref <- 100*(sfcWind_ref_i/mean(sfcWind_ref_i[34:63])-1)
sfcWind_ref_Year <- 1958:2020
sfcWind_diff_1 <- mean(sfcWind_ref[4:33])

print("sfcWind. ref periods:")
print(round(sfcWind_diff_1,2))
print(round(mean(sfcWind_ref[34:63]),2))
print("###")

points(2006,0,pch=19)
points(1976,sfcWind_diff_1,pch=19)

# lines(sfcWind_ref$Year,smth(sfcWind_ref$GR0,window = 45, method = "gaussian"),lwd=2)
lines(sfcWind_ref_Year[10:54],smoooth_wkw(sfcWind_ref,h=9)[10:54],lwd=2+k)

# sm2 <- ksmooth(sfcWind_ref$Year,sfcWind_ref$GR0,"normal",bandwidth = 24)
