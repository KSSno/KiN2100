...
if(READ)
{
  ...
  for(i in 1:nmod)
  {
    ...
    Zrcp26[1,i] <- mean(ncvar_get(nc,"tas")*mask,na.rm=T)
    nc_close(nc)
    nc <- nc_open(filelist_tas_rcp26_ff[i])
    Zrcp26[2,i] <- mean(ncvar_get(nc,"tas")*mask,na.rm=T)
    nc_close(nc)
    
    #RCP45
    nc <- nc_open(filelist_tas_rcp45_nf[i])
    Zrcp45[1,i] <- mean(ncvar_get(nc,"tas")*mask,na.rm=T)
    nc_close(nc)
    nc <- nc_open(filelist_tas_rcp45_ff[i])
    Zrcp45[2,i] <- mean(ncvar_get(nc,"tas")*mask,na.rm=T)
    nc_close(nc)
    
    #SSP370
    nc <- nc_open(filelist_tas_ssp370_nf[i])
    Zssp370[1,i] <- mean(ncvar_get(nc,"tas")*mask,na.rm=T)
    nc_close(nc)
    nc <- nc_open(filelist_tas_ssp370_ff[i])
    Zssp370[2,i] <- mean(ncvar_get(nc,"tas")*mask,na.rm=T)
    nc_close(nc)
  }
}
...
#SSP370
plot(Zssp370[1,],xlim=c(0,45),ylim=c(0,8),pch="+")
points(22:41,Zssp370[2,],pch="+")

#RCP4.5
plot(Zrcp45[1,],xlim=c(0,45),ylim=c(0,8),pch="+")
points(22:41,Zrcp45[2,],pch="+")

#RCP2.6
plot(Zrcp26[1,],xlim=c(0,45),ylim=c(0,8),pch="+")
...
png("/home/andreasd/Documents/KiN2100_2023/Indices/plots/tas/tas_changes_ANN.png",width = k*1280, height = k*640,pointsize=k*28)
...
abline(h=seq(-3,5,1),col="grey",lty=3,lwd=k)
...
tas_ref <- read.table("/lustre/storeB/project/KSS/kin2100_2024/Region_series/regionserier.GR/tama_1901-2021_ANN.csv",header=T,sep=";")
...
lines(tas_ref$Year[10:111],smoooth_wkw(tas_ref$GR0,h=9)[10:111],lwd=2+k)
