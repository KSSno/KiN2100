...
if(READ)
{
  Zrcp26 <- Zrcp45 <- Zssp370 <- array(NA,c(2,20,11)) #NF/FF, 20 runs, 11 areas
  ...
  for(i in 1:nmod)
  {
    ...
    for (j in 1:11)
    {
      ...
      Zrcp26[1,i,j] <- mean(ncvar_get(nc1,"pr")*regmask,na.rm=T)
      Zrcp26[2,i,j] <- mean(ncvar_get(nc2,"pr")*regmask,na.rm=T)
      
      Zrcp45[1,i,j] <- mean(ncvar_get(nc3,"pr")*regmask,na.rm=T)
      Zrcp45[2,i,j] <- mean(ncvar_get(nc4,"pr")*regmask,na.rm=T)
      
      Zssp370[1,i,j] <- mean(ncvar_get(nc5,"pr")*regmask,na.rm=T)
      Zssp370[2,i,j] <- mean(ncvar_get(nc6,"pr")*regmask,na.rm=T)
    }
...
for (j in 1:11)
{
  ...
  #SSP370
  plot(Zssp370[1,,j],xlim=c(0,45),ylim=c(-40,40),pch="+")
  points(22:41,Zssp370[2,,j],pch="+")
  abline(h=0)
  
  #RCP4.5
  plot(Zrcp45[1,,j],xlim=c(0,45),ylim=c(-40,40),pch="+")
  points(22:41,Zrcp45[2,,j],pch="+")
  abline(h=0)
  
  #RCP2.6
  plot(Zrcp26[1,,j],xlim=c(0,45),ylim=c(-40,40),pch="+")
  ...
  png(paste0("/home/andreasd/Documents/KiN2100_2023/Indices/plots/pr/pr_changes_ANN_RR_region_",j,".png"),width = k*1280, height = k*640,pointsize=k*28)
  ...
  yl <- c(-30,30) #round(range(Zssp370[2,,j],Zrcp26[2,,j],Zrcp45[2,,j],pr_diff_1,pr_diff_2, pr_diff_3),-1) + c(-10,10)
  ...
  abline(h=seq(-100,100,10),col="grey",lty=3,lwd=k)
  ...
  pr_ref <- read.table("/lustre/storeB/project/KSS/kin2100_2024/Region_series/regionserier.nye.KR/rra_1901-2021_ANN.csv",header=T,sep=";")-100
  ...
  # print(round(mean(pr_ref[71:100,j+1])))
  # print("###")
