...
if(READ)
{
  ...
  for (s in 1:4)
  {
    ...
    for(i in 1:nmod)
    {
      ...
      #RCP26
      nc <- nc_open(filelist_tas_rcp26_nf[i])
      Zrcp26[1,i,s] <- mean(ncvar_get(nc,"tas")*mask,na.rm=T)
      nc_close(nc)
      nc <- nc_open(filelist_tas_rcp26_ff[i])
      Zrcp26[2,i,s] <- mean(ncvar_get(nc,"tas")*mask,na.rm=T)
      nc_close(nc)
      
      #RCP45
      nc <- nc_open(filelist_tas_rcp45_nf[i])
      Zrcp45[1,i,s] <- mean(ncvar_get(nc,"tas")*mask,na.rm=T)
      nc_close(nc)
      nc <- nc_open(filelist_tas_rcp45_ff[i])
      Zrcp45[2,i,s] <- mean(ncvar_get(nc,"tas")*mask,na.rm=T)
      nc_close(nc)
      
      #SSP370
      nc <- nc_open(filelist_tas_ssp370_nf[i])
      Zssp370[1,i,s] <- mean(ncvar_get(nc,"tas")*mask,na.rm=T)
      nc_close(nc)
      nc <- nc_open(filelist_tas_ssp370_ff[i])
      Zssp370[2,i,s] <- mean(ncvar_get(nc,"tas")*mask,na.rm=T)
      nc_close(nc)
    }
  }
}
...
for (s in 1:4)
{
  ...
  #SSP370
  plot(Zssp370[1,,s],xlim=c(0,45),ylim=c(0,8),pch="+")
  points(22:41,Zssp370[2,,s],pch="+")

  #RCP4.5
  plot(Zrcp45[1,,s],xlim=c(0,45),ylim=c(0,8),pch="+")
  points(22:41,Zrcp45[2,,s],pch="+")

  #RCP2.6
  plot(Zrcp26[1,,s],xlim=c(0,45),ylim=c(0,8),pch="+")
  ...
  png(paste0("/home/andreasd/Documents/KiN2100_2023/Indices/plots/tas/tas_changes_",seas[s],".png"),width = k*1280, height = k*640,pointsize=k*28)
  ...
  # tas_ref <- read.table(paste0("/lustre/storeB/project/KSS/kin2100_2024/Region_series/regionserier.GR/tama_1901-2021_",seas[s],".csv"),header=T,sep=";")
  ## !!!!
  ## USE abs. values and calculate deviation as the deviation files for DJF are wrong
  tas_ref <- read.table(paste0("/lustre/storeB/project/KSS/kin2100_2024/Region_series/regionserier.GR/tam_1901-2021_",seas[s],".csv"),header=T,sep=";")
  tas_diff_1 <- mean(tas_ref[1:30,2])-mean(tas_ref[91:120,2])
  tas_diff_2 <- mean(tas_ref[31:60,2])-mean(tas_ref[91:120,2])
  tas_diff_3 <- mean(tas_ref[61:90,2])-mean(tas_ref[91:120,2])
  # tas_diff_1 <- mean(tas_ref$GR0[1:30])
  # tas_diff_2 <- mean(tas_ref$GR0[31:60])
  # tas_diff_3 <- mean(tas_ref$GR0[61:90])
  ...
  lines(tas_ref$Year[10:111],smoooth_wkw(tas_ref[,2]-mean(tas_ref[91:120,2]),h=9)[10:111],lwd=2+k)
