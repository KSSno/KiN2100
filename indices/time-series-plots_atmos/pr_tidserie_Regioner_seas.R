...
load("/lustre/storeB/project/KSS/kin2100_2024/Indices/pr_changes_regions_seasons.RData")
...
if(READ)
{
  Zrcp26 <- Zrcp45 <- Zssp370 <- array(NA,c(2,20,11,4)) #NF/FF, 20 runs, 11 areas, 4 seasons
  
  # Region masks
  nc <- nc_open("/lustre/storeB/project/KSS/kin2100_2024/geoinfo/rr_region_2021d_Nr_OnSeNorgeGrid.nc")
  mask <- ncvar_get(nc,"RR-region")
  nc_close(nc)
  ...
  for (s in 1:4)
  {
    ...
    ### Filelists
    # near future
    filelist_pr_rcp26_nf <- list.files("/lustre/storeB/project/KSS/kin2100_2024/Indices/30yr-diff-files_for_plotting/pr/seasonal/",paste0("30yrmean_nf-change.*rcp26.*",seas[s]),full.names = T)
    filelist_pr_rcp45_nf <- list.files("/lustre/storeB/project/KSS/kin2100_2024/Indices/30yr-diff-files_for_plotting/pr/seasonal/",paste0("30yrmean_nf-change.*rcp45.*",seas[s]),full.names = T)
    filelist_pr_ssp370_nf <- list.files("/lustre/storeB/project/KSS/kin2100_2024/Indices/30yr-diff-files_for_plotting/pr/seasonal/",paste0("30yrmean_nf-change.*ssp370.*",seas[s]),full.names = T)
    
    # far future
    filelist_pr_rcp26_ff <- list.files("/lustre/storeB/project/KSS/kin2100_2024/Indices/30yr-diff-files_for_plotting/pr/seasonal/",paste0("30yrmean_ff-change.*rcp26.*",seas[s]),full.names = T)
    filelist_pr_rcp45_ff <- list.files("/lustre/storeB/project/KSS/kin2100_2024/Indices/30yr-diff-files_for_plotting/pr/seasonal",paste0("30yrmean_ff-change.*rcp45.*",seas[s]),full.names = T)
    filelist_pr_ssp370_ff <- list.files("/lustre/storeB/project/KSS/kin2100_2024/Indices/30yr-diff-files_for_plotting/pr/seasonal",paste0("30yrmean_ff-change.*ssp370.*",seas[s]),full.names = T)
    ...
    for(i in 1:nmod)
    {
      ...
      #RCP26
      nc1 <- nc_open(filelist_pr_rcp26_nf[i])
      nc2 <- nc_open(filelist_pr_rcp26_ff[i])
      #RCP45
      nc3 <- nc_open(filelist_pr_rcp45_nf[i])
      nc4 <- nc_open(filelist_pr_rcp45_ff[i])
      
      #SSP370
      nc5 <- nc_open(filelist_pr_ssp370_nf[i])
      nc6 <- nc_open(filelist_pr_ssp370_ff[i])
      ...
      for (j in 1:11)
      {
        ...
        Zrcp26[1,i,j,s] <- mean(ncvar_get(nc1,"pr")*regmask,na.rm=T)
        Zrcp26[2,i,j,s] <- mean(ncvar_get(nc2,"pr")*regmask,na.rm=T)
        
        Zrcp45[1,i,j,s] <- mean(ncvar_get(nc3,"pr")*regmask,na.rm=T)
        Zrcp45[2,i,j,s] <- mean(ncvar_get(nc4,"pr")*regmask,na.rm=T)
        
        Zssp370[1,i,j,s] <- mean(ncvar_get(nc5,"pr")*regmask,na.rm=T)
        Zssp370[2,i,j,s] <- mean(ncvar_get(nc6,"pr")*regmask,na.rm=T)
      }
  ...
  table_nf <- array(NA,c(4,9,11))
  table_ff <- array(NA,c(4,9,11))
...
for (j in 1:11)
{
  ...
  for (s in 1:4)
  {
    ...
    #SSP370
    plot(Zssp370[1,,j,s],xlim=c(0,45),ylim=c(-50,50),pch="+")
    points(22:41,Zssp370[2,,j,s],pch="+")
    abline(h=0)
    
    #RCP4.5
    plot(Zrcp45[1,,j,s],xlim=c(0,45),ylim=c(-50,50),pch="+")
    points(22:41,Zrcp45[2,,j,s],pch="+")
    abline(h=0)
    
    #RCP2.6
    plot(Zrcp26[1,,j,s],xlim=c(0,45),ylim=c(-50,50),pch="+")
    ...
    png(paste0("/home/andreasd/Documents/KiN2100_2023/Indices/plots/pr/pr_changes_",seas[s],"_RR_region_",j,".png"),width = k*1280, height = k*640,pointsize=k*28)
    ...
    abline(h=seq(-50,50,10),col="grey",lty=3,lwd=k)
    ...
    # pr_ref <- read.table(paste0("/lustre/storeB/project/KSS/kin2100_2024/Region_series/regionserier.nye.KR/rra_1901-2021_",seas[s],".csv"),header=T,sep=";")-100
    # pr_ref$Year <- pr_ref$Year + 100
    # pr_diff_1 <- mean(pr_ref[1:30,j+1])
    # pr_diff_2 <- mean(pr_ref[31:60,j+1])
    # pr_diff_3 <- mean(pr_ref[61:90,j+1])
    
    ## !!!!
    ## USE abs. values and calculate deviation as the deviation files for DJF are wrong
    pr_ref <- read.table(paste0("/lustre/storeB/project/KSS/kin2100_2024/Region_series/regionserier.nye.KR/rr_1901-2021_",seas[s],".csv"),header=T,sep=";")
    pr_diff_1 <- 100*(mean(pr_ref[1:30,j+1])/mean(pr_ref[91:120,j+1])-1)
    pr_diff_2 <- 100*(mean(pr_ref[31:60,j+1])/mean(pr_ref[91:120,j+1])-1)
    pr_diff_3 <- 100*(mean(pr_ref[61:90,j+1])/mean(pr_ref[91:120,j+1])-1)
    ...
    # lines(pr_ref$Year[10:111],smoooth_wkw(pr_ref[,j+1],h=9)[10:111],lwd=2+k)
    lines(pr_ref$Year[10:111],smoooth_wkw(100*(pr_ref[,j+1]/mean(pr_ref[91:120,j+1])-1),h=9)[10:111],lwd=2+k)
...
#save(Zrcp26,Zrcp45,Zssp370,table_nf,table_ff,file="~/Documents/KiN2100_2023/Indices/pr_changes_regions_seasons.RData")
