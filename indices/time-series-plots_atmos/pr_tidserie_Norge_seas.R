...
if(READ)
{
  ...
  for (s in 1:4)
  {
    ...
    for(i in 1:nmod)
    {
      ...
      Zrcp26[1,i,s] <- mean(ncvar_get(nc,"pr")*mask,na.rm=T)
      nc_close(nc)
      nc <- nc_open(filelist_pr_rcp26_ff[i])
      Zrcp26[2,i,s] <- mean(ncvar_get(nc,"pr")*mask,na.rm=T)
      nc_close(nc)
      
      #RCP45
      nc <- nc_open(filelist_pr_rcp45_nf[i])
      Zrcp45[1,i,s] <- mean(ncvar_get(nc,"pr")*mask,na.rm=T)
      nc_close(nc)
      nc <- nc_open(filelist_pr_rcp45_ff[i])
      Zrcp45[2,i,s] <- mean(ncvar_get(nc,"pr")*mask,na.rm=T)
      nc_close(nc)
      
      #SSP370
      nc <- nc_open(filelist_pr_ssp370_nf[i])
      Zssp370[1,i,s] <- mean(ncvar_get(nc,"pr")*mask,na.rm=T)
      nc_close(nc)
      nc <- nc_open(filelist_pr_ssp370_ff[i])
      Zssp370[2,i,s] <- mean(ncvar_get(nc,"pr")*mask,na.rm=T)
      nc_close(nc)
    }
  }
}
...
for (s in 1:4)
{
  ...
  png(paste0("/home/andreasd/Documents/KiN2100_2023/Indices/plots/pr/pr_changes_",seas[s],".png"),width = k*1280, height = k*640,pointsize=k*28)
  ...
  # pr_ref <- read.table(paste0("/lustre/storeB/project/KSS/kin2100_2024/Region_series/regionserier.GR/rra_1901-2021_",seas[s],".csv"),header=T,sep=";")-100
  ...
  pr_ref <- read.table(paste0("/lustre/storeB/project/KSS/kin2100_2024/Region_series/regionserier.GR/rr_1901-2021_",seas[s],".csv"),header=T,sep=";")
  pr_diff_1 <- 100*(mean(pr_ref[1:30,2])/mean(pr_ref[91:120,2])-1)
  pr_diff_2 <- 100*(mean(pr_ref[31:60,2])/mean(pr_ref[91:120,2])-1)
  pr_diff_3 <- 100*(mean(pr_ref[61:90,2])/mean(pr_ref[91:120,2])-1)
  ...
  lines(pr_ref$Year[10:111],smoooth_wkw(100*(pr_ref[,2]/mean(pr_ref[91:120,2])-1),h=9)[10:111],lwd=2+k)
