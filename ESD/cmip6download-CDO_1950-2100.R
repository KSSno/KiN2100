  ...
  ## this is used to download the fil es that are too big for R and cause a problem
        ...
        for (i in 1:n) {
          ...
          if ( (sum(imatch > 0)==0) |  (overwrite) ) {
            ...
            if (tolower(substr(test,1,5))=="error") print('Skip this file') else { 
              ...
              system('rm tmp.cmip6.*.nc') 
              ## Fix bad time intervals
              tim <- getinterval(file.path(path,ncname),param)
              nstr <- decipher(ncname)
              datafraction <- (max(year(tim))-min(year(tim)))
              cperiod <- paste(range(year(tim)),collapse='-')
              newname <- sub(nstr[4],cperiod,ncname)
              print(paste('rename ',ncname,newname))
              file.rename(file.path(path,ncname),file.path(path,newname))
            }
